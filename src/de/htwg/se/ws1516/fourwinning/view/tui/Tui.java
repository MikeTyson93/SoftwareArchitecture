...
public class Tui implements IObserver {
	...
	public String playGame(String command) {
		...
		LOGGER.info("Um alle Spielst�nde anzuschauen, geben sie 'show' ein");
		LOGGER.info("Um einen Spielstand zu l�schen, geben Sie 'delete #savegame' ein");
		...
		if ("redo".equals(currentColumnString)) {
			...
			if (!success){
				LOGGER.info("Sie haben einen ung�ltigen Spielnamen angegeben!");
				return "load";
			}
			this.eins = spiel.getPlayerOne();
			this.zwei = spiel.getPlayerTwo();
			...
			// save game
			LOGGER.info("Nun m�ssen sie noch den Namen des Spielstandes eingeben, welchen Sie speichern wollen.");
			String savegame = eingabe.nextLine();
			if (!spiel.saveToDB(savegame)){
				LOGGER.info("Sie haben einen schon belegten Namen angegeben. Wollen Sie den Spielstand �berschreiben? y/n");
				String yesNo = eingabe.nextLine();
				while (yesNo != "y" && yesNo != "n"){
					LOGGER.info("Falsche Eingabe. Spielstand �berschreiben? y/n");
			...
			LOGGER.info("Nun m�ssen sie noch den Namen des Spielstandes eingeben, welchen Sie l�schen wollen.");
			String deleteGame = eingabe.nextLine();
			if (!spiel.deleteFromDB(deleteGame)){
				LOGGER.info("Spielstand konnte nicht gel�scht werden.");
				return "delete";
			}
			LOGGER.info("Spielstand gel�scht.");
			return "delete";
			...
			LOGGER.warning("Ung�ltige Eingabe!");
			return "show";
		}
